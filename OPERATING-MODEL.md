# Operating Model — Healthcare Payer/Provider (HPP)
...
| 2 | **Who approves consequential actions?** | The framework-enforced human-approval gate (bound, single-use, SoD tokens) — the *mechanism* | Names the approver role and staffs it (**Denials Manager / HIM (under the BAA)**); the human makes every consequential decision | Accel: R (mechanism) · Customer: A (decision) |
...
| 6 | **Who answers audit requests?** | Append-only + WORM audit, control matrices (NIST/OWASP), the assurance packet, this operating model | Produces the evidence to the auditor; owns the HIPAA / payer / HITRUST audit response (**Compliance / GRC**) | Accel: C · Customer: R/A |
...
3. Business reviewer (Denials Manager / HIM (under the BAA)) and the AI/ML owner approve through the customer's change process.
...
- [ ] Approver role staffed (Denials Manager / HIM (under the BAA)); SoD enforced (`STRICT_APPROVAL=1` / bound approvals only).
...
- [ ] GRC owner for HIPAA / payer / HITRUST audit; audit-evidence export tested from the append-only trail.
