...
function titleSlide(pptx, code, title, sub){
  ...
  s.addText(code,{x:0.6,y:0.7,w:12,h:0.5,fontSize:16,bold:true,color:ORANGE,fontFace:FT});
...
function statSlide(pptx, code, heading, issueLine, stats, src){
  const s=pptx.addSlide(); s.background={color:INK}; accentBar(s);
  s.addText(heading,{x:0.6,y:0.5,w:12,h:0.7,fontSize:30,bold:true,color:WHITE,fontFace:FTL});
...
function resultsSlide(pptx, code, brightLine, why, outcome, src){
  const s=pptx.addSlide(); s.background={color:INK}; accentBar(s);
  s.addText("Why This Is Defensible",{x:0.6,y:0.5,w:12,h:0.7,fontSize:30,bold:true,color:WHITE,fontFace:FTL});
  // bright line strip
