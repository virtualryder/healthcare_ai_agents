...
def test_declares_agp_1_0():
    pkg = importlib.import_module("hpp_agent_platform")
    assert getattr(pkg, "AEGIS_GOVERNANCE_PATTERN_VERSION", None) == "1.0"


def test_token_budget_module_present():
    m = importlib.import_module("hpp_agent_platform.budget")
    assert hasattr(m, "BudgetMeter")


def test_core_control_modules_import():
    for mod in ("hpp_agent_platform.mcp_gateway.gateway", "hpp_agent_platform.mcp_gateway.policy",
                "hpp_agent_platform.mcp_gateway.audit"):
