...
tool, carrying the acting workforce user's context. The connector presents it to
the system of record (EHR, clearinghouse, payer portal, patient accounting); it
expires in seconds. This mirrors what Amazon Bedrock AgentCore Identity issues
(or API Gateway + STS): ephemeral, least-privilege, attributable — the control
that lets a HIPAA Security Rule / HITRUST auditor trace every PHI system touch to
a named person and a single tool (the "minimum necessary" access standard,
45 CFR 164.502(b), enforced at the token).
