# Security Evidence Pack — HPP Agent 01 — Revenue-Cycle Denials & Appeals
...
Effective permission = **agent grant ∩ user entitlement** (role: `DENIALS_SPECIALIST`). Consequential acts are
...
|---|---|---|
| `pas.get_claim` | granted | read |
| `pas.get_account` | granted | read |
| `pas.update_case` | granted | write · human approval |
| `clearinghouse.validate_claim` | granted | read |
| `payer.check_claim_status` | granted | read |
| `payer.submit_appeal` | granted | write · human approval |
| `coding.validate_codes` | granted | read |
| `coding.check_medical_necessity` | granted | read |
| `ehr.get_clinical_docs` | granted | read |
| `kb.search_policy` | granted | read |
| `clearinghouse.submit_claim` | **withheld from the agent** | DENIALS_MANAGER / BILLER (a biller submits claims) |
| `payer.issue_determination` | **withheld from the agent** | no agent holds it — UM medical-director decision |
...
Data class for this hero: **PHI under a signed AWS BAA**.
