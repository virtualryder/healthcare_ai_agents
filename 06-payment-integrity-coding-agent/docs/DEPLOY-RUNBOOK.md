# Deploy Runbook — Agent 06 (Payment Integrity & Coding)
...
companion deployment handbook is `../../deliverables/agent-handbooks/06-payment-integrity-coding-HANDBOOK.md`.

## 1. Prove it locally (no AWS, no API key)
```bash
cd 06-payment-integrity-coding-agent && EXTRACT_MODE=demo python demo/demo_run.py     # from repo root
PYTHONPATH=../platform_core:..:.  python -m pytest tests -q
```

## 2. Deploy into a new AWS account
```bash
scripts/build_lambdas.sh 06-payment-integrity-coding
...
`infra/cloudformation/params/06-payment-integrity-coding.json`. Terraform: `terraform apply -var-file=envs/06-payment-integrity-coding.tfvars`.

## 3. Human-gate smoke test
1. Seed a Cognito user with the agent's reviewer role and sign in.
2. Drive a gated action; confirm it **suspends at the Payment-Integrity Reviewer gate** and an item lands in the HITL table.
