# Deploy Runbook — Agent 05 (Utilization Management)
...
companion deployment handbook is `../../deliverables/agent-handbooks/05-utilization-management-HANDBOOK.md`.

## 1. Prove it locally (no AWS, no API key)
```bash
cd 05-utilization-management-agent && EXTRACT_MODE=demo python demo/demo_run.py     # from repo root
PYTHONPATH=../platform_core:..:.  python -m pytest tests -q
```

## 2. Deploy into a new AWS account
```bash
scripts/build_lambdas.sh 05-utilization-management
...
`infra/cloudformation/params/05-utilization-management.json`. Terraform: `terraform apply -var-file=envs/05-utilization-management.tfvars`.

## 3. Human-gate smoke test
1. Seed a Cognito user with the agent's reviewer role and sign in.
2. Drive a gated action; confirm it **suspends at the Medical Director gate** and an item lands in the HITL table.
