# Deploy Runbook — Agent 07 (Care Management & Population Health)
...
companion deployment handbook is `../../deliverables/agent-handbooks/07-care-management-pophealth-HANDBOOK.md`.

## 1. Prove it locally (no AWS, no API key)
```bash
cd 07-care-management-pophealth-agent && EXTRACT_MODE=demo python demo/demo_run.py     # from repo root
PYTHONPATH=../platform_core:..:.  python -m pytest tests -q
```

## 2. Deploy into a new AWS account
```bash
scripts/build_lambdas.sh 07-care-management-pophealth
...
`infra/cloudformation/params/07-care-management-pophealth.json`. Terraform: `terraform apply -var-file=envs/07-care-management-pophealth.tfvars`.

## 3. Human-gate smoke test
1. Seed a Cognito user with the agent's reviewer role and sign in.
2. Drive a gated action; confirm it **suspends at the Care Manager gate** and an item lands in the HITL table.
