...
# care-manager gate, returns pending state; resume() with an approval runs finalize.
# ============================================================
...
def run_until_gate(initial: Dict[str, Any]) -> Dict[str, Any]:
    ...
    s.update(nodes.load_patient(s))
    s.update(nodes.check_consent(s))
    s.update(nodes.identify_gaps(s))
    s.update(nodes.fairness_screen(s))

    for _ in range(2):
        s.update(nodes.draft_artifacts(s))
        s.update(nodes.compliance_check(s))
        if nodes.routing_decision(s) == "care_manager_gate":
            break
